...
def shortest(n, _edges):
    ...
    while h: #len(h) > 0:
        ...
        if v == n-1: # target is popped (fixed)
            print("with decrease-key: time %.2f secds, V %d, E %d, pushes %d, pops %d" % (time.time() - t, n, len(_edges), pushes, pops))
...
def shortest_heapq(n, _edges):
    ...
    while h: #len(h) > 0:
        ...
        if v not in popped: # the first time v turns black
            popped.add(v)
            if v == n-1: # target is popped (fixed)
                print("without decrease-key: time %.2f secs, V %d, E %d, pushes %d, pops %d" % (time.time() - t, n, len(_edges), pushes, pops))
                return d, solution(v, back)
            for (u, cost) in edges[v]:
                newd = d + cost
...
if __name__ == "__main__":
    ...
    big = generate_seq(5000, 200000, 1)
